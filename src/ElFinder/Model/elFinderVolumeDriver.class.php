...
abstract class elFinderVolumeDriver
{
    ...
    public function mount(array $opts)
    {
        ...
        $this->id = $this->driverId . (!empty($this->options['id'])
                ? $this->options['id'] : elFinder::$volumesCnt++) . '_';
        $this->root = $this->_normpath($this->options['path']);
        $this->separator = isset($this->options['separator'])
            ? $this->options['separator'] : DIRECTORY_SEPARATOR;

        // default file attribute
        $this->defaults = array(
            'read' => isset($this->options['defaults']['read'])
                    ? !!$this->options['defaults']['read'] : true,
            'write' => isset($this->options['defaults']['write'])
                    ? !!$this->options['defaults']['write'] : true,
            'locked' => false,
            'hidden' => false
        );
        ...
        $this->uploadAllow = isset($this->options['uploadAllow'])
        && is_array(
        ...
        $this->uploadDeny = isset($this->options['uploadDeny'])
        && is_array(
        ...
        if (is_string(
            ...
            $parts = explode(
                ...
                isset($this->options['uploadOrder'])
                    ? $this->options['uploadOrder'] : 'deny,allow'
            );
        ...
        $this->disabled = isset($this->options['disabled'])
        && is_array(
        ...
        if ($this->mimeDetect == 'internal' && !self::$mimetypesLoaded) {
            ...
            $file = false;
            if (!empty($this->options['mimefile'])
                && file_exists(
        ...
        $this->rootName = empty($this->options['alias']) ? $this->_basename(
            $this->root
        ) : $this->options['alias'];
        ...
        $this->treeDeep
            = $this->options['treeDeep'] > 0 ? (int)$this->options['treeDeep']
            : 1;
        $this->tmbSize
            =
            $this->options['tmbSize'] > 0 ? (int)$this->options['tmbSize'] : 48;
        ...
        if (!empty($this->options['archivers']['extract'])
            && is_array(
                $this->options['archivers']['extract']
            )
        ) {
    ...
    public function options($hash)
    {
        return array(
            ...
            'archivers' => array(
                ...
                'create' => is_array($this->archivers['create']) ? array_keys(
                        $this->archivers['create']
                    ) : array(),
                'extract' => is_array($this->archivers['extract']) ? array_keys(
    ...
    public function tree($hash = '', $deep = 0, $exclude = '')
    {
        ...
        $dirs = $this->gettree(
            ...
            $exclude ? $this->decode($exclude) : null
        );
    ...
    public function tmb($hash)
    {
        ...
        if (isset($stat['tmb'])) {
            return $stat['tmb'] == "1" ? $this->createTmb($path, $stat)
                : $stat['tmb'];
        }
        return false;
    }
    ...
    public function mkdir($dst, $name)
    {
        ...
        $this->clearcache();
        return ($path = $this->_mkdir($path, $name)) ? $this->stat($path)
            : false;
    }
    ...
    public function mkfile($dst, $name)
    {
        ...
        $this->clearcache();
        return ($path = $this->_mkfile($path, $name)) ? $this->stat($path)
            : false;
    }
    ...
    public function paste($volume, $src, $dst, $rmSrc = false)
    {
        ...
        if ($stat) {
            if ($this->options['copyOverwrite']) {
                ...
                if ($volume == $this
                    && ($test == @
                            $file['target'] || $test == $this->decode($src))
                ) {
        ...
        if ($volume == $this) {
            ...
            if ($this->_inpath($destination, $source)) {
                return $this->setError(
                    elFinder::ERROR_COPY_INTO_ITSELF,
                    $errpath
                );
            }
        ...
        if (($path = $this->copyFrom($volume, $src, $destination, $name))
            == false
        ) {
            return false;
        }
    ...
    public function putContents($hash, $content)
    {
        ...
        $this->clearcache();
        return $this->_filePutContents($path, $content) ? $this->stat($path)
            : false;
    }
    ...
    public function extract($hash)
    {
        ...
        $this->clearcache();
        return ($path = $this->_extract($path, $archiver)) ? $this->stat($path)
            : false;
    }
    ...
    protected function attr($path, $name, $val = null)
    {
        ...
        for ($i = 0, $c = count($this->attributes); $i < $c; $i++) {
            ...
            if (isset($attrs[$name]) && isset($attrs['pattern'])
                && preg_match(
                    $attrs['pattern'],
                    $path
                )
        ...
        return $perm === null ? (is_null($val) ? $this->defaults[$name] : $val)
            : !!$perm;
    }
    ...
    protected function allowCreate($dir, $name)
    {
        ...
        for ($i = 0, $c = count($this->attributes); $i < $c; $i++) {
            ...
            if (isset($attrs['write']) && isset($attrs['pattern'])
                && preg_match($attrs['pattern'], $testPath)
            ) {
                $perm = $attrs['write'];
            }
        }
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if ($this->options['utf8fix'] && $this->options['utf8patterns']
        ...
        $stat['read'] = intval(
            $this->attr(
                $path,
                'read',
                isset($stat['read']) ? !!$stat['read'] : null
            )
        );
        $stat['write'] = intval(
            $this->attr(
                $path,
                'write',
                isset($stat['write']) ? !!$stat['write'] : null
            )
        );
        if ($root) {
            $stat['locked'] = 1;
    ...
    static protected function mimetypeInternalDetect($path)
    {
        ...
        return isset(elFinderVolumeDriver::$mimetypes[$ext])
            ? elFinderVolumeDriver::$mimetypes[$ext] : 'unknown';
    ...
    protected function gettree($path, $deep, $exclude = '')
    {
        ...
        foreach ($this->dirsCache[$path] as $p) {
            ...
            if ($stat && empty($stat['hidden']) && $p != $exclude
                && $stat['mime'] == 'directory'
            ) {
                $dirs[] = $stat;
    ...
    protected function canCreateTmb($path, $stat)
    {
        ...
        && strpos($path, $this->tmbPath) === false
        // do not create thumnbnail for thumnbnail
        && $this->imgLib
        && strpos($stat['mime'], 'image') === 0
        && ($this->imgLib == 'gd' ?
            $stat['mime'] == 'image/jpeg' || $stat['mime'] == 'image/png'
