...
class elFinderVolumeMySQL extends elFinderVolumeDriver
{
    ...
    protected function init()
    {
        ...
        $this->db
            = new mysqli($this->options['host'], $this->options['user'], $this->options['pass'], $this->options['db'], $this->options['port'], $this->options['socket']);
    ...
    public function search($q, $mimes)
    {
        ...
        $sql
            = 'SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, 0 AS dirs
				FROM %s AS f 
        ...
        if (($res = $this->query($sql))) {
            while ($row = $res->fetch_assoc()) {
                if ($this->mimeAccepted($row['mime'], $mimes)) {
                    ...
                    if (($stat = $this->updateCache($id, $row))
                        && empty($stat['hidden'])
                    ) {
                        $result[] = $stat;
                    }
    ...
    public function resize(
        ...
        if (($res = $this->query(
                'SELECT content FROM ' . $this->tbf . ' WHERE id="' . $path
                . '"'
            ))
            && ($r = $res->fetch_assoc())
        ) {
    ...
    protected function getParents($path)
    {
        ...
        while ($path) {
            if ($file = $this->stat($path)) {
                ...
                $path = isset($file['phash']) ? $this->decode($file['phash'])
                    : false;
            }
        }
    ...
    {
        return ($stat = $this->stat($path)) ? ($stat['phash'] ? $this->decode(
            $stat['phash']
        ) : $this->root) : false;
    }
    ...
    {
        $sql = 'SELECT id FROM ' . $this->tbf . ' WHERE parent_id="' . $dir
            . '" AND name="' . $this->db->real_escape_string($name) . '"';
    ...
    {
        return
            ($stat = $this->stat($path)) && isset($stat['dirs']) ? $stat['dirs']
                : false;
    }
    ...
    {
        return ($stat = $this->stat($path)) && isset($stat['width'])
        && isset($stat['height']) ? $stat['width'] . 'x' . $stat['height'] : '';
    }
    ...
    {
        return $this->make($path, $name, 'directory') ? $this->_joinPath(
            $path,
            $name
        ) : false;
    }
    ...
    {
        return $this->make($path, $name, 'text/plain') ? $this->_joinPath(
            $path,
            $name
        ) : false;
    }
    ...
    protected function _move($source, $targetDir, $name)
    {
        ...
        return $this->query($sql) && $this->db->affected_rows > 0 ? $source
            : false;
    }
