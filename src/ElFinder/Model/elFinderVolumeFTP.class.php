...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function init()
    {
        ...
        // normalize root path
        $this->root = $this->options['path'] = $this->_normpath(
            $this->options['path']
        );
    ...
    protected function configure()
    {
        ...
        if (!empty($this->options['tmpPath'])) {
            if ((is_dir($this->options['tmpPath'])
                    || @mkdir(
                        $this->options['tmpPath'],
                        0755,
                        true
                    ))
    ...
    protected function connect()
    {
        if (!($this->connect = ftp_connect(
            ...
            return $this->setError(
                'Unable to connect to FTP server ' . $this->options['host']
            );
        }
        if (!ftp_login(
            $this->connect,
            ...
            $this->umount();
            return $this->setError(
                'Unable to login into ' . $this->options['host']
            );
        }
    ...
    protected function parseRaw($raw)
    {
        ...
        if ($this->ftpOsUnix) {
            ...
            $stat['name'] = $name;
            $stat['mime']
                = substr(strtolower($info[0]), 0, 1) == 'd' ? 'directory'
                : $this->mimetype($stat['name']);
    ...
    protected function parsePermissions($perm)
    {
        ...
        $read = ($owner && $parts[0] == 'r') || $parts[4] == 'r'
            || $parts[7] == 'r';

        return array(
            'read' => $parts[0] == 'd' ? $read
                    && (($owner && $parts[3] == 'x') || $parts[6] == 'x'
                        || $parts[9] == 'x') : $read,
            'write' => ($owner && $parts[2] == 'w') || $parts[5] == 'w'
                || $parts[8] == 'w'
        );
    }
    ...
    {
        return strpos($this->mimetype($path), 'text/') === 0 ? FTP_ASCII
            : FTP_BINARY;
    }
    ...
    {
        return
            $path == $this->root ? '' : substr($path, strlen($this->root) + 1);
    }
    ...
    protected function _abspath($path)
    {
        return $path == $this->separator
            ? $this->root : $this->root . $this->separator . $path;
    }
    ...
    {
        return $this->rootName . ($path == $this->root
        ...
        if (is_array($raw) && count($raw) > 1
            ...
            foreach ($parts as $part) {
                ...
                switch ($key) {
                    case 'type':
                        $stat['mime']
                            = strpos($val, 'dir') !== false ? 'directory'
                            : $this->mimetype($path);
                        break;
            ...
            if (isset($stat['chmod'])) {
                ...
                $stat['read'] = $stat['mime'] == 'directory' ? $read
                    && (($owner && $perm[0][2]) || $perm[1][2]
                        || $perm[2][2]) : $read;
                $stat['write']
                    = ($owner && $perm[0][1]) || $perm[1][1] || $perm[2][1];
                unset($stat['chmod']);
    ...
    protected function _extract($path, $arc)
    {
        ...
        $command = escapeshellcmd(
            $arc['cmd'] . ' ' . $arc['argc'] . ' "' . $basename . '"'
        );
        exec($command, $output, $return_value);
        unlink($basename);
        if ($return_value != 0) {
            $this->setError(
                elFinder::ERROR_EXTRACT_EXEC,
                'Command failed ' . escapeshellarg($command)
            );
            $this->deleteDir($tmpDir); //cleanup
            return false;
        }

        $filesToProcess = elFinderVolumeFTP::listFilesInDirectory(
            $tmpDir,
            true
        );
        if (!$filesToProcess) {
    ...
    protected function _archive($dir, $files, $name, $arc)
    {
        ...
        if (!ftp_put($this->connect, $remoteArchiveFile, $path, FTP_BINARY)) {
            $this->setError(
                elFinder::ERROR_FTP_UPLOAD_FILE,
                $remoteArchiveFile
            );
            $this->deleteDir($tmpDir); //cleanup
            return false;
    ...
    private function ftp_scan_dir($remote_directory)
    {
        ...
        foreach ($buff as $str) {
            ...
            switch ($type) {
                ...
                default:
                    $remote_file_path
                        = $remote_directory . DIRECTORY_SEPARATOR . $info[8];
                    $item = array();
    ...
    private function ftp_download_files(
        ...
        if (!isset($contents)) {
            $this->setError(
                elFinder::ERROR_FTP_DOWNLOAD_FILE,
                $remote_directory
            );
            return false;
        }
        foreach ($contents as $item) {
            ...
            foreach ($files as $file) {
                if ($remote_directory . DIRECTORY_SEPARATOR . $file
                    == $item['path']
                    || strstr(
                        $item['path'],
                        $remote_directory . DIRECTORY_SEPARATOR . $file
                        . DIRECTORY_SEPARATOR
                    )
                ) {
                    $drop = false;
            ...
            $local_path
                = $dest_local_directory . DIRECTORY_SEPARATOR . $relative_path;
            switch ($item['type']) {
                case 'd':
    ...
    private function deleteDir($dirPath)
    {
        if (!is_dir($dirPath)) {
            ...
            foreach (
                array_reverse(
                    elFinderVolumeFTP::listFilesInDirectory($dirPath, false)
                ) as $path
            ) {
            ...
            if (!in_array($file, $excludes)) {
                ...
                if (is_link($path)) {
                    ...
                    if (is_dir($path)) {
                        ...
                        $subs = elFinderVolumeFTP::listFilesInDirectory(
                            $path,
                            $omitSymlinks,
