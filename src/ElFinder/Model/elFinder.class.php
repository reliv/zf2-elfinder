...
class elFinder
{
    ...
    public function __construct($opts)
    {
        ...
        foreach ($opts['roots'] as $i => $o) {
            $class
                = 'elFinderVolume' . (isset($o['driver']) ? $o['driver'] : '');
    ...
    {
        return $this->loaded && isset($this->commands[$cmd])
        && method_exists(
            $this,
            $cmd
        );
        ...
        if ($this->debug || !empty($args['debug'])) {
            $result['debug'] = array(
                ...
                'memory' => (function_exists('memory_get_peak_usage') ?
                    ...
                    . ceil(memory_get_usage() / 1024) . 'Kb / ' . ini_get(
                        'memory_limit'
                    ),
    ...
    {
        return isset($_SESSION['elFinderNetVolumes'])
        && is_array(
            $_SESSION['elFinderNetVolumes']
        ) ? $_SESSION['elFinderNetVolumes'] : array();
    }
    ...
    protected function upload($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        $files = isset($args['FILES']['upload'])
        && is_array(
            $args['FILES']['upload']
        ) ? $args['FILES']['upload'] : array();
        $result = array(
            'added' => array(),
            'header' => empty($args['html']) ? false
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            if (($file = $volume->upload($fp, $target, $name, $tmpname))
                === false
            ) {
                $result['warning'] = $this->error(
                    self::ERROR_UPLOAD_FILE,
    ...
    protected function paste($args)
    {
        ...
        foreach ($targets as $target) {
            ...
            if (($file = $dstVolume->paste($srcVolume, $target, $dst, $cut))
                == false
            ) {
    ...
    protected function extract($args)
    {
        ...
        $mimes
            =
            !empty($args['mimes']) && is_array($args['mimes']) ? $args['mimes']
                : array();
    ...
    {
        $type = $args['type'];
        $targets = isset($args['targets']) && is_array($args['targets'])
            ? $args['targets'] : array();
