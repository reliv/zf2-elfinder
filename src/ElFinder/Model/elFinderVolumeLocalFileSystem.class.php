...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        ...
        $this->options['maxArcFilesSize']
            = 0; // max allowed archive files size (0 - no limit)
    }
    ...
    protected function configure()
    {
        ...
        if ($root['read'] && !$this->tmbURL && $this->URL) {
            if (strpos($this->tmbPath, $this->root) === 0) {
                $this->tmbURL = $this->URL . str_replace(
                        DIRECTORY_SEPARATOR,
                        '/',
        ...
        if (!empty($this->options['quarantine'])) {
            $this->quarantine = $this->root . DIRECTORY_SEPARATOR
                . $this->options['quarantine'];
    ...
    protected function readlink($path)
    {
        ...
        if ($this->_inpath($atarget, $this->aroot)) {
            return $this->_normpath(
                $this->root . DIRECTORY_SEPARATOR . substr(
                    $atarget,
                    strlen($this->aroot) + 1
                )
            );
    ...
    protected function _checkArchivers()
    {
        ...
        if ($c == 0) {
            $arcs['create']['application/x-7z-compressed']
                = array('cmd' => '7za', 'argc' => 'a', 'ext' => '7z');
            $arcs['extract']['application/x-7z-compressed']
                = array('cmd' => '7za', 'argc' => 'e -y', 'ext' => '7z');
    ...
    protected function _extract($path, $arc)
    {
        ...
        if ($this->quarantine) {
            $dir = $this->quarantine . DIRECTORY_SEPARATOR . str_replace(
                    ' ',
                    '_',
                    microtime()
                ) . basename($path);
            ...
            if ($this->options['maxArcFilesSize'] > 0
            ...
            if (count($ls) == 1) {
                ...
                if (!$this->_mkdir(dirname($path), $name)
                    || !copy(
                        $path,
                        $archive
                    )
    ...
    protected function _archive($dir, $files, $name, $arc)
    {
        ...
        $cmd
            =
            $arc['cmd'] . ' ' . $arc['argc'] . ' ' . escapeshellarg($name) . ' '
            . implode(' ', $files);
        $this->procExec($cmd, $o, $c);
        chdir($cwd);
